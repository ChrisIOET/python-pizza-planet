...
engine = db.create_engine("sqlite:///{}".format(os.path.join(BASE_DIR, "pizza.sqlite")))
...
def random_customer_generator():
    ...
    client_list = list(
        set(
            [
                (
                    f"{names[random.randint(0, len(names)-1)]} {lastnames[random.randint(0, len(lastnames)-1)]}",
                    f"calle{client_address[random.randint(0, len(client_address)-1)]}",
                    f"{ random.randint(20000000,90000000) }",
                    f"+{random.randint(100,  999) }, { random.randint(10,99) } { random.randint(100,999)} { random.randint(1000,9999)}",
                )
                for _ in range(50)
            ]
        )
...
def generate_random_number():
    return float("{:.2f}".format(round(random.uniform(1, 9), 2)))
...
def ingredients_random_generator():
    ...
    return [
        [ingredients[i], generate_random_number()] for i in range(0, len(ingredients))
    ]


def size_random_generator():
    sizes = ["small", "medium", "large", "xl", "xxl"]
    # return [[sizes[i], generate_random_number()] for i in range(0, len(sizes))]
    return [{
        "size": size,
...
def beverage_random_generator():
    beverages = ["licor", "whisky", "water", "red bull", "fanta", "coke", "monster"]
    return [[beverages[i], generate_random_number()] for i in range(0, len(beverages))]
...
def ingredients_table_populator():
    ...
    for i in range(1, len(ingredients) + 1):
        session.add(Ingredient(name=ingredients[i - 1][0], price=ingredients[i - 1][1]))
    session.commit()
    session.close()
